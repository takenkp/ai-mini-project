...
1. 분기처리를 할때, join과 같은 더미노드를 빼기보다는, agent 내부에서 await를 통해서 처리를 하면 더 간결하게 처리할 수 있을 것 같다라는 피드백을 받았습니다.
2. ToxicClauseAgent에서, 10개의 독소조항을 뽑기위해 프롬프트를 열심히 짜봤지만, 어느 기점부터 2~3개의 독소조항만 뽑아내는 것을 확인할 수 있었습니다. 이 부분에 대해서는 MS의 https://arxiv.org/pdf/2505.06120 논문을 참고한다면, 일정 대화창 내에서는 cache를 Refresh 해주는게 좋다는 걸 확인할수 있었습니다. 이는 __pycache__를 지우고 다시 사용한다면 충분히 달라질 수 있을것이라 판단됩니다.